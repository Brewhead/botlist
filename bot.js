...
REmbed.first().edit(new Discord.MessageEmbed().setColor('GREEN').setDescription(`Bu bot \`${client.users.cache.get(Log.executor.id).tag}\` tarafından kabul edildi./accepted By`))
...
.setColor('BLUE')
.setTitle('🎉 Müjde!')
.setDescription(`🎉 Tebrikler! \`${member.guild.name}\` Adlı Sunucuda Sırada Olan/Sorting on a Named Server \`${client.users.cache.get(member.id).tag}\` Adlı Botunuz/Your Named Bot \`${client.users.cache.get(Log.executor.id).tag}\` Tarafından Onaylandı! (Sunucudan Çıkarsan Atılacaktır.)/Approved by! (Your bot will be kicked out if you leave the server).`)
...
client.channels.cache.get(ayarlar.BOTLog).send(`**<@${BOTEkleyen}> Adlı Kullanıcının/This User's \`${client.users.cache.get(member.id).tag}\` Adlı Sistemde Onay Bekleyen Botu/Bot Waiting for Approval in the System Named \`${client.users.cache.get(Log.executor.id).tag} [${db.fetch(`Count_${Log.executor.id}`) || 1}]\` Tarafından Kabul Edildi./Accepted By**`)
db.add(`Count_${Log.executor.id}`,1)
db.delete(`Durum_${member.id}`)
member.roles.add(ayarlar.BOTRol) // BOT Rol ID
} else {
member.roles.add('894214768896905284') // Üye Rol ID
}
})
// [ ----------------------------------------------] \\
// [ ----------------------------------------------] \\
...
client.user.setActivity('+botekle +addbot',{ type: 'WATCHING'})
...
client.channels.cache.get(ayarlar.BOTLog).send(`\`${member.user.tag}\` Sunucudan Ayrıldı./Left Server. \`${User.tag}\` Adındaki Botunu Attım/I Threw Your Bot Named. 👌`)
...
**\`${message.guild.name}\` Sunucusunda Sırada Bekleyen/Waiting in Queue on Server. \`${Client.tag}\` Adındaki Botunuz/Your Bot Named \`${Collected.first().author.tag}\` Tarafından/By \`${Collected.first().content || 'Belirtilmedi/Not Specified'}\` Sebebiyle Reddedildi./Rejected Due To.**`)
            client.channels.cache.get(ayarlar.BOTLog).send(`**<@${Bilgi.Gönderen}> Adlı Kullanıcının/This User's. \`${Client.tag}\` Adlı Sistemde Onay Bekleyen Botu/Bot Waiting for Approval in the System Named. \`${Collected.first().author.tag}\` Tarafından \`${Collected.first().content || 'Belirtilmedi/Not Specified'}\` Sebebiyle Reddedildi./Rejected Due To.**`)
            })
            })
    })
    })
...
client.login(ayarlar.token).catch(() => console.error('(node:8295) UnhandledPromiseRejectionWarning: Hata [GEÇERSİZ_TOKEN]: Geçersiz token tespit edildi.'))
